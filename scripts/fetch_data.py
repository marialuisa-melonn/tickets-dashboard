 import psycopg2
  ...
  ---
  docs/index.html — 4 cambios puntuales

  En el <script>, hacer estos reemplazos:

  1. Cambiar const WEEKS a let con array vacío (buscar la línea que empieza con const WEEKS=[):
  // REEMPLAZAR toda la const WEEKS=[...] por:
  let WEEKS = [];

  2. Cambiar const SENTIMENT a let con array vacío (buscar const SENTIMENT=[):
  // REEMPLAZAR toda la const SENTIMENT=[...] por:
  let SENTIMENT = [];

  3. Agregar estas dos funciones justo antes de // ── STATE):
  function buildWeeks(rows) {
    const MONTHS = ['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];
    const byWeek = {};
    rows.forEach(r => {
      if (!byWeek[r.semana]) byWeek[r.semana] = {};
      byWeek[r.semana][r.seller] = {
        o: r.ordenes, ts: r.tickets_hubspot, tb: r.tickets_orbita,
        ai: r.ai_resolvio, esc: r.ai_escalo, wait: r.en_espera
      };
    });
    return Object.keys(byWeek).sort().map(d => {
      const dt = new Date(d + 'T00:00:00');
      return { w: dt.getDate() + '-' + MONTHS[dt.getMonth()], d, s: byWeek[d] };
    });
  }

  function buildSentiment(rows) {
    const COLOR = { Frustrado:'#E67E22', Negativo:C.red, Preocupado:'#C49A0A',
                    Insatisfecho:'#E74C3C', Positivo:C.green, Neutral:'#7B7B9B' };
    const total = rows.reduce((s, r) => s + r.total, 0);
    return rows.map(r => ({
      l: r.grupo,
      v: total > 0 ? Math.round(r.total / total * 100) : 0,
      c: COLOR[r.grupo] || C.muted
    }));
  }

  4. Reemplazar el bloque final de inicialización (las últimas líneas del script, que empiezan con SELLERS.forEach y terminan con renderAll()):
  SELLERS.forEach(s => {
    const o = document.createElement('option');
    o.value = s; o.textContent = s;
    document.getElementById('fSeller').appendChild(o);
  });

  fetch('./data.json')
    .then(r => r.json())
    .then(data => {
      WEEKS = buildWeeks(data.tendencia_por_seller || []);
      SENTIMENT = buildSentiment(data.sentimiento || []);
      const upd = data.actualizado_en || '—';
      document.getElementById('updBadge').textContent = 'Act: ' + upd;
      document.getElementById('updLbl').textContent =
        'Datos al ' + upd + ' · ' + filteredWeeks().length + ' semanas';
      renderAll();
    })
    .catch(() => {
      document.getElementById('updBadge').textContent = 'Sin datos';
      renderAll();
    });

  5. En renderAll(), quitar las dos líneas que setean updBadge y updLbl hardcodeadas (ya no hacen falta):
  // ELIMINAR estas dos líneas dentro de renderAll():
  document.getElementById('updBadge').textContent='Act: 22 abr 2026 06:00';
  document.getElementById('updLbl').textContent='Datos al 22 abr 2026 · '+filteredWeeks().length+' semanas';
